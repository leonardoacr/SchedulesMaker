...
export const updateNoteInDB = async (username: string, day: string, oldTime: string, oldNote: string, newTime: string, newNote: string): Promise<void> => {
    console.log('Updating note in DB: ' + '\n Old Note and Time: ' + oldNote + ' ' + oldTime + '\n New Note and Time: ' + newNote + ' ' + newTime);
    try {
