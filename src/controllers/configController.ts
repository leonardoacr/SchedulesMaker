...
export const getConfig = async (req: Request, res: Response) => {
    ...
    console.log('Getting data from frontend: ' + backgroundImage + ' ' + backgroundTheme);
    const filter = { username: username };
    const update = { $set: { config: { backgroundImage, backgroundTheme } } };

    try {
        await Schedules.findOneAndUpdate(filter, update);
        console.log('Updated config data for user: ' + backgroundImage + ' ' + backgroundTheme)
        res.json({ success: true });
    } catch (error) {
        console.error(error);
...
export const sendConfig = async (req: Request, res: Response) => {
    ...
    const configData: Schedule | null = await Schedules.findOne(query, projection);
    ...
    if (configData && configData.config.length > 0) {
        ...
        if (sendConfigData) {
            console.log('Sending data to frontend: ' + JSON.stringify(sendConfigData))
